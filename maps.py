...
for year in range(2001, 2021):
    ...
    file_paths = glob(os.path.join('/discover/nobackup/projects/giss_ana/users/kmezuman/GFED5/BA', pattern))
...
def Data_grid_stat(target_data):
    ...
    monthly_means_data.to_netcdf('/discover/nobackup/projects/giss_ana/users/kmezuman/GFED5/GFED5_meanBA_2001_2020.nc')
    ...
    months_name_list = [ 'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November','December']
    ...
    for variable in ['Total','Crop','Peat','Defo']:
        ...
        data_array = xr.DataArray(my_array, coords=[('latitude', latitude_coords), ('longitude', longitude_coords), ('months',months)])
    ...
    monthly_means_data.to_netcdf('/discover/nobackup/projects/giss_ana/users/kmezuman/GFED5/GFED5_monthmaxBA_2001_2020.nc')
...
import pandas as pd
import xarray as xr
target_data = xr.open_dataset('/discover/nobackup/projects/giss_ana/users/kmezuman/GFED5/combined_monthly_data.nc',engine = 'netcdf4')
target_data['BA'] = target_data['BA_shrub']+target_data['BA_tree']+target_data['BA_grass']
time_coords_str = target_data['time'].values
time_coords = pd.to_datetime(time_coords_str, format='%b%Y')  # Assuming 'JAN1997' format
...
def Data_grid_stat_nudged(target_data):
    ...
    for variable in ['BA', 'BA_shrub', 'BA_grass', 'BA_tree']:
        ...
        for i, months in enumerate(months_of_interest):
            # Select the burned area values for the current month set and variable
            burned_area = target_data[variable].sel(time=target_data['time'].dt.month.isin(months))
            print(burned_area)
            # Calculate the mean burned area for each grid cell
            mean_burned_area = burned_area.mean(dim='time')
            #<CROP/PEAT/DEFO/NAT/TOTAL>_meanBA_DJF,
    ...
    # Save the xarray dataset to a new NetCDF file
    monthly_means_data.to_netcdf('/discover/nobackup/projects/giss_ana/users/kmezuman/GFED5/Model_meanBA_1997_2019_nudged.nc')
    ...
    for variable in ['BA','BA_shrub','BA_grass','BA_tree']:
        ...
        for i, month in enumerate(months):
            ...
            #get data for months across the years
            burned_area = variable_data.sel(time=target_data['time'].dt.month.isin(month))
    ...
    monthly_means_data.to_netcdf('/discover/nobackup/projects/giss_ana/users/kmezuman/GFED5/Model_monthmaxBA_2001_2019_nudged.nc')
...
def Data_grid_stat_model(target_data):
    ...
    # Save the xarray dataset to a new NetCDF file
    monthly_means_data.to_netcdf('/discover/nobackup/projects/giss_ana/users/kmezuman/GFED5/Model_meanBA_1997_2019.nc')
    ...
    monthly_means_data.to_netcdf('/discover/nobackup/projects/giss_ana/users/kmezuman/GFED5/Model_monthmaxBA_2001_2019.nc')
    #data_array.to_netcdf("new_monthly.nc")
    #month_mean.to_netcdf('month_mean.nc')
