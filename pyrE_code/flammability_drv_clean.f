...
!#endif
     ...
     &                  grid%j_strt_halo:grid%j_stop_halo) :: sddarr2d
      ...
      real*8 :: lai
      real*8 :: laisum, covsum, laico, covpatch
...
! update the precipitation running average
     ...
     &          fuel_entcell=fuel_entcell )
             ...
             do ncov=1,N_COVERTYPES  !Currently: 1 cohort/patch
               laisum = laisum + fuel_entcell(ncov)%area * 
     &             fuel_entcell(ncov)%cofuel(1)%LAI
               covsum = covsum + fuel_entcell(ncov)%area
             enddo
             if (covsum>0.d0) then
                !print *, 'lai1',lai
                lai= laisum/covsum !give dummy weighted avg for now
                !print *, 'lai2', lai
             else
                lai = 0.d0
             endif
