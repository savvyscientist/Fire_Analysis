...
!@auth Keren Mezuman 
      ...
      real*8, intent(INOUT) :: burnt_area
      ...
      real*8 :: new_burnt_area,pvt_area
      real*8 :: umax
      integer :: nv
      !if there are no fires to create new BA and no old BA to recover
      if ((saveFireCount <= 0.d0) .AND. (burnt_area <= 0.d0)) return
...
!@var a average fire spread area (m^2)
        ...
        burnt_area =  min(new_burnt_area , pvt_area) 
        select case(ent_cover_names(nv))
          case('arid_shrub','cold_shrub')
     ...
     &         'ever_br_late','ever_nd_late')
        ...
        ! Only use the first matching type
        exit
      end do
...
!@var avg see ravg_lai(i,j) the running average prec returned
      ...
      subroutine calculate_fire_count(i,j,ncov)
...
! Putting that all together to get the fire count rate (fire/m2/s):
        saveFireCount(ncov,i,j)=tuneFireCount*flammability(ncov,i,j)
     &    *(CtoG+humanIgn)*nonSuppressFrac(i,j)
...
! saveFireCount = [#fire/m2]
        saveFireCount(ncov,i,j)=flammability(ncov,i,j)*mfcc*
     &    yearsPerSecond
      endif
...
!@var EPFCByVegType emission factor (kg/#fire)
      ...
      do j=J_0,J_1
        do i=I_0,I_1
          ...
          do nv=1,N_COVERTYPES
            emisPerFire = pvt(nv)*fearth(i,j)*EPFBVT(nv)) ! [kg/#fire
          ! saveFireCount = [#fire/m2/sec] gridbox
            sfc_src(i,j,n,ns) = sfc_src(i,j,n,ns) + emisPerFire*saveFireCount(nv,i,j)
          end do ! nv
        end do ! i
      end do   ! j
#endif  /* TRACERS_ON */
